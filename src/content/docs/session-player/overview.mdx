...
When a session ends with an **error** status, the player facilitates efficient debugging. The failed session can be compared against the original baseline recording to visually identify changes.
...
- **Pass Status**: If the scenario passed, the player opens displaying only the original recording by default.
- **Error Status**: If the latest run failed, the player automatically opens in **[Side-by-Side](./side-by-side-compare)** comparison mode.
:::
